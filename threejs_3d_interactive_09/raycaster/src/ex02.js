...
export default function example() {
  ...
  function draw() {
    ...
    boxMesh.position.y = Math.sin(time);
    torusMesh.position.y = Math.cos(time);
    boxMesh.material.color.set("plum");
    torusMesh.material.color.set("lime");
    const origin = new THREE.Vector3(0, 0, 100);
    // const direction = new THREE.Vector3(0, 0, -1); // 방향이기 때문에, 거리는(-100) 상관없다.
    const direction = new THREE.Vector3(0, 0, -100);
    console.log("1", direction.length());
    direction.normalize();
    console.log("2", direction.length());
