...
export default function example() {
  ...
  const floorBody = new CANNON.Body({
    mass: 0,
    position: new CANNON.Vec3(0, 0.14, 0),
    shape: floorShape,
  ...
  const preventDefaultClick = new PreventDragClick(canvas);
  // Raycaster
  ...
  canvas.addEventListener("click", (e) => {
    if (preventDefaultClick.mouseMoved) return;
    mouse.x = (e.clientX / canvas.clientWidth - 0.5) * 2;
    mouse.y = -(e.clientY / canvas.clientHeight - 0.5) * 2;
    checkIntersects();
  });
